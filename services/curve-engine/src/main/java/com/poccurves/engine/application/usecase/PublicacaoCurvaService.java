...
public class PublicacaoCurvaService {
     ...
     * {@code curvaImportadaMesmaData} — ver {@link com.poccurves.engine.application.validator.TesteComparacaoCurvaImportada}.
     */
    public void processarPedidoConstrucao(String curveCode, LocalDate referenceDate, String curveMomentStr, UUID runId, UUID executionId) {
        try {
            ...
            if (veredito.aprovadaSemBloqueioReprovado()) {
                ...
                curvaPublicadaEventPublisher.notificarFalha(executionId, "Curva reprovada na validação: " + bloqueantesReprovados + " teste(s) bloqueante(s) reprovado(s)");
            }
        } catch (Exception e) {
            log.error("Falha ao processar construção: curveCode={}, executionId={}", curveCode, executionId, e);
            curvaPublicadaEventPublisher.notificarFalha(executionId, e.getMessage());
            return;
        }
    }
}
