...
/**
 * Interpolação cúbica suave em espaço logarítmico da taxa.
 * É mais suave que LOG_LINEAR porque o spline cúbico natural considera TODOS
 * os vértices simultaneamente para determinar a curvatura em cada trecho,
 * não só o par mais próximo.
 */
public final class InterpoladorLogCubico implements Interpolador {

    public static final String IDENTIFICADOR = "LOG_CUBICO";
    ...
    public BigDecimal taxaEm(List<Vertice> verticesOrdenados, int prazoDiasUteis, MathContext mathContext) {
        ...
        if (verticesOrdenados.size() < 2) {
            throw new IllegalArgumentException("interpolação log-cúbica exige ao menos 2 vértices");
        }
        ...
        for (Vertice v : verticesOrdenados) {
            if (v.taxa().compareTo(BigDecimal.ZERO) <= 0) {
                throw new IllegalArgumentException("interpolação log-cúbica exige taxa > 0, vértice inválido: " + v);
            }
        ...
        for (Vertice v : verticesOrdenados) {
            ...
            ys.add(RoundingPolicy.lnRaw(v.taxa(), mathContext));
        }

        BigDecimal lnTaxaInterpolada = SplineCubicaNatural.avaliar(xs, ys, BigDecimal.valueOf(prazoDiasUteis), mathContext);
        return RoundingPolicy.expRaw(lnTaxaInterpolada, mathContext);
    }
}
