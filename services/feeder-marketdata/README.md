# Feeder de Market Data (`feeder-marketdata`)
...
`feeder-marketdata` não é um serviço de pé como os demais do `compose.yaml` — ele roda uma aquisição por execução e termina ("pull agendado", ver "Extensibilidade" abaixo). Por isso fica atrás de um profile dedicado, não sobe com `up.sh`/`up.ps1`:

```bash
cd deploy/podman
ACQUISITION_REFERENCE_DATE=2026-08-21 \
podman compose -f compose.yaml --profile feeder-on-demand run --rm feeder-marketdata
```
...
### Scripts NPM
Execute a partir do diretório do serviço (`services/feeder-marketdata/`):
...
O build da imagem de container é definido em `services/feeder-marketdata/Containerfile`.
...
```bash
podman build -f services/feeder-marketdata/Containerfile -t feeder-marketdata .
```
