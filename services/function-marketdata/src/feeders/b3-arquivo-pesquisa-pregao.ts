...
export class FeederB3ArquivoPesquisaPregao implements Feeder {
  ...
  async acquire(params: AcquisitionParams): Promise<AcquisitionResult> {
    ...
    const url = urlDownloadB3(this.config.prefixoArquivo, params.referenceDate, 'zip');
    ...
    switch (desempacotado.tipo) {
      case 'ZIP_EXTERNO_VAZIO':
        // Sinal real e confirmado de "arquivo ainda não publicado" para este endpoint.
    ...
    const conteudoXml = desempacotado.dados;
