...
/**
 * Teste de integração real: verifica {@link FeederAcquisitionClient} contra
 * um feeder-marketdata-http de verdade (caminhos PUBLISHED/NO_DATA — exige o
 * container `feeder-marketdata-http` do compose de pé, porta 8091 exposta) e
 ...
 */
class FeederAcquisitionClientIT {
    ...
    void acionarRetornaNoDataParaDataFuturaSemPublicacao() {
        FeederAcquisitionClient client = new FeederAcquisitionClient(clienteParaPorta(8091));

        FeederAcquisitionClient.ResultadoAquisicao resultado = client.acionar(
    ...
    void acionarDesserializaCorpoFailedEmRespostaHttp502SemLancarExcecao() throws Exception {
        ...
        // feeder-marketdata-http (confirmado via azure-function-handler.ts: status
        ...
        FeederAcquisitionClient client = new FeederAcquisitionClient(clienteParaPorta(porta));
