...
/**
 * Cliente HTTP para o serviço {@code feeder-marketdata-http} (Node.js).
 * <p>
 * Executa o comando de aquisição remota {@code POST /acquire} e recebe
 * o reporte de resultado (PUBLISHED, NO_DATA, FAILED).
 */
@Component
public class FeederAcquisitionClient implements FeederAcquisitionPort {
    ...
    public FeederAcquisitionClient(@Qualifier("feederAcquisitionRestClient") RestClient restClient) {
        this.restClient = restClient;
    }

    /**
     * Aciona a aquisição do conjunto de dados no feeder-marketdata-http.
     * <p>
     * Resposta HTTP 502 do feeder representa um resultado de negócio do tipo {@code FAILED}
        ...
        try {
            AcquisicaoResponse resposta = restClient.post()
                    ...
                    .onStatus(status -> status.value() == 502, (req, res) -> {
                        // 502 é FAILED com corpo válido do feeder, não erro de transporte de rede.
                    })
