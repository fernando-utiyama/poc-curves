...
**Revisto — ver `openspec/changes/raw-file-blob-storage`**: a decisão acima foi revertida conscientemente. Os feeders de arquivo (B3 PR/IN, ANBIMA — e o de TS/`b3-additional-curves`) passam a gravar o conteúdo bruto completo em blob storage (Azurite local / Azure Blob Storage em produção) e publicar **um único evento por aquisição**, referenciando o blob (`blobContainer`/`blobPath`) em vez de embuti-lo em blocos. O motivo da reversão não é que os riscos apontados aqui deixaram de existir — retenção do blob e referência pendurada continuam sendo trade-offs reais, aceitos explicitamente (ver design.md daquela mudança, seção Risks/Trade-offs) — mas que a POC passou a querer refletir o padrão de arquivamento/reprocessamento de arquivo de origem que a plataforma de produção usa, o que pesa mais que o custo de infraestrutura adicional (que, neste caso, já era zero: SQL Server já era obrigatório, e Azurite já é só mais um container Podman). `curve.build.requested.v1`/blocos de dado individual continuam existindo como conceito (`sequencia`/`totalBlocos` no envelope) — só passam a valer sempre `1` para os datasets migrados, não removidos do modelo.

*O que a quebra em blocos custava*: a atomicidade que "um arquivo = uma mensagem" dava de graça. Isso volta de graça com o blob — a aquisição inteira grava um blob e publica uma única referência, sem depender de contagem de blocos para os datasets migrados.

*O que ela ganhava além do tamanho, e deixa de ganhar*: o processor começava a gravar enquanto o feeder ainda lia o arquivo (ganho de orçamento na janela crítica), e ninguém segurava o arquivo inteiro em memória do lado do feeder. Com blob, o feeder grava o arquivo inteiro de uma vez antes de publicar — aceito porque o tamanho dos arquivos desta POC (na casa de centenas de MB) não torna isso um gargalo real, e o ganho de simplicidade (sem lógica de corte estrutural, sem contagem de blocos) compensa.
